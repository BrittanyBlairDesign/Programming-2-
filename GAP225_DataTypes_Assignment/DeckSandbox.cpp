...
void DeckSandbox::Discard()
{
	...
	PlayingCard* pDiscard = m_pHand[cardIndex];
...
{
	PlayingCard* pDiscardedCards[52]{ nullptr };
	int numDiscarded = 0;

	// We have to pull from the discard one by one
	while (true)
	{
		PlayingCard* pCard = m_deck.DrawFromDiscardPile();
		if (pCard != nullptr)
		{
	...
	for (size_t i = 0; i < numDiscarded; ++i)
	{
